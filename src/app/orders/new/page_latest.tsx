...
export default function NewOrderPage() {
  ...
  // ✅ Local AI-like upsell simulation
  useEffect(() => {
    if (!items.length) {
      setUpsells([]);
      return;
    ...
    setTimeout(() => {
      const selectedNames = items
        .map((it) => beverages.find((b) => b.id === it.beverage_id)?.name)
        .filter(Boolean);

      const related = beverages.filter(
        (b) =>
          selectedNames.some((sel) => b.name !== sel) &&
          selectedNames.some((sel) =>
            b.category
              .toLowerCase()
              .includes(sel?.split(" ")[0]?.toLowerCase() || "")
          )
      );
    ...
    <div className="p-6">
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded border border-blue-200">
        <h3 className="font-semibold mb-2">
          ✨ Mix & Upsell Suggestions (Local AI)
        </h3>
        {loadingUpsell && <p className="text-gray-500">Analyzing order...</p>}
        {!loadingUpsell && upsells.length === 0 && <p>No suggestions yet.</p>}
