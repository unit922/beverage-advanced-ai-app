...
export default function NewBeveragePage() {
  ...
  function handleChange(e: any) {
    ...
    // auto recalc stock shots
    if (name === "bottle_volume_ml" || name === "shot_size_ml" || name === "stock_bottles") {
      ...
      updated.stock_shots = bottles > 0 && shot > 0 ? Math.floor((bottles * volume) / shot) : 0;
    }
  ...
  async function save() {
    try {
      ...
      alert("❌ Error saving beverage: " + err.message);
    }
  }
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="number" name="price_bottle" value={form.price_bottle} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Price (Shot)</label>
          <input type="number" name="price_shot" value={form.price_shot} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Price (Rocks)</label>
          <input type="number" name="price_rocks" value={form.price_rocks} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Price (Mix)</label>
          <input type="number" name="price_mix" value={form.price_mix} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Price (Glass)</label>
          <input type="number" name="price_glass" value={form.price_glass} onChange={handleChange} className="border p-2 w-full" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        <div>
          ...
          <input type="number" name="bottle_volume_ml" value={form.bottle_volume_ml} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Shot Size (ml)</label>
          <input type="number" name="shot_size_ml" value={form.shot_size_ml} onChange={handleChange} className="border p-2 w-full" />
        </div>
        <div>
          <label>Stock (Bottles)</label>
          <input type="number" name="stock_bottles" value={form.stock_bottles} onChange={handleChange} className="border p-2 w-full" />
        </div>
      </div>

      <div className="mt-4">
        <label>Calculated Stock (Shots)</label>
        <input type="number" readOnly value={form.stock_shots} className="border p-2 w-full bg-gray-100" />
      </div>
